...
// Import config setting for WebSocket relay server URI
import { WEBSOCKET_RELAY_SERVER } from "@/config";
...
const useAudioHandlers = () => {
  ...
  const handleAudioRecord = async (
    ...
    console.log("handleAudioRecord called, current state:", { isRecording, hasWebSocket: !!webSocketClient });
    ...
    if (!isRecording && webSocketClient) {
      if (!audioRecorderRef.current) {
        ...
        audioRecorderRef.current = new Recorder(async (buffer) => {
          console.log("Audio chunk captured, size:", buffer.byteLength, "bytes");
...
const ChatInterface = () => {
  const [endpoint, setEndpoint] = useState(`${WEBSOCKET_RELAY_SERVER.protocol}${WEBSOCKET_RELAY_SERVER.uri}${WEBSOCKET_RELAY_SERVER.route}` || "ws://localhost:8080/realtime");
  ...
  const toggleRecording = async () => {
    console.log("toggleRecording called, current state:", { isRecording, isConnected });
    try {
      const newRecordingState = await handleAudioRecord(
        webSocketClient.current,
        isRecording
      );
      console.log("Recording state updated:", { oldState: isRecording, newState: newRecordingState });
